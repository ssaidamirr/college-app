...
def fetch_ai_scores(universities):
    ...
    schema = {
        "type": "OBJECT",
        "description": "An object where each key is a university name and the value is its scores.",
        "additionalProperties": {
            "type": "OBJECT",
            "properties": ai_factor_properties,
        },
    }
...
if st.sidebar.button("Generate AI Rankings", type="primary", use_container_width=True, disabled=len(valid_universities) < 2):
    with st.spinner("AI is researching and ranking your universities... This may take a moment."):
        ...
        raw_ai_scores = fetch_ai_scores(valid_universities)
        
        if raw_ai_scores:
            # Normalize AI scores (match keys)
            normalized_ai_scores = {}
            for uni_name in valid_universities:
                lower_uni_name = uni_name.lower()
                found_key = next((key for key in raw_ai_scores if lower_uni_name in key.lower()), None)
                
                if found_key:
                    normalized_ai_scores[uni_name] = raw_ai_scores[found_key]
                else:
